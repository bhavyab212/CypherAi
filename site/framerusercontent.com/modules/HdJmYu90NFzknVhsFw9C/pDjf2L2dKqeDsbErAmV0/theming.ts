import {
    ...
    nightOwl,
    sandpackDark,
} from "../modules/codesandbox_sandpack_themes.js"
import { isObject, isDarkColor } from "./utils.ts"
import { FontProperties } from "./types.ts"
...
export type ThemeKeys = (typeof themeKeys)[number]

export const themes: Record<ThemeKeys, unknown> = {
    amethyst,
    aquaBlue,
    atomDark,
    atomLight,
...
export function getVariableTheme(font: FontProperties) {
    return {
        colors: {
...
export function getVariablesForTheme(
    theme: string,
    ...
    // Collect all colors and turn them into CSS variables.
    if (isObject(colors)) {
        ...
        for (const name in syntax) {
            ...
            if (isObject(value)) {
                for (const style in value) {
                    result[`--cb-syntax-${style}-${name}-${mode}`] =
                        value[style]
                }
            } else {
    ...
    if (background) {
        ...
        background = result[`--cb-colors-surface1-${mode}`] as string
    }
