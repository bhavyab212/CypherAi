import {
  ...
  nightOwl,
  sandpackDark,
} from "/framerusercontent.com/modules/c4BjG1nmVImY1EzTuV13/B7rGpsdWsN6NgevmHZzi/codesandbox_sandpack_themes.js";
import {
  isObject,
  isDarkColor,
} from "/framerusercontent.com/modules/IdMJ4CymCWCh4cOBkZlb/eelWnWuz7D3MwsDAYYYK/utils.js";
import { clampRGB } from "framer";
const magic = {
  colors: {
    surface1: "#f6f6f8",
    surface2: "#f6f6f8",
...
export function getVariablesForTheme(theme, background, mode) {
  const result = {};
  const { colors, syntax } = themes[theme] || {}; // Collect all colors and turn them into CSS variables.
  if (isObject(colors)) {
  ...
  if (background) {
    result[
      `--cb-colors-surface1-${mode}`
    ] = `var(--cb-custom-background, ${background})`;
    result["--cb-background-rgb"] = clampRGB(background);
  } else {
    background = result[`--cb-colors-surface1-${mode}`];
  } // This is important to show scrollbars and selections with the correct color.
  result[`--cb-color-scheme-${mode}`] = isDarkColor(background)
    ? "dark"
    : "light";
  return result;
}
export const __FramerMetadata__ = {
  exports: {
    themes: { type: "variable", annotations: { framerContractVersion: "1" } },
    lightThemeVariables: {
      type: "variable",
      annotations: { framerContractVersion: "1" },
    },
    ThemeKeys: { type: "tsType", annotations: { framerContractVersion: "1" } },
    getVariableTheme: {
      type: "function",
      annotations: { framerContractVersion: "1" },
    },
