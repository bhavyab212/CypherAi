 ...
 */ export default function Animator(props) {
  ...
  /* Empty State */ let customShape = /*#__PURE__*/ _jsxs("div", {
  ...
  if (hasChildren) {
    ...
    /* Shape transition properties */ /* Dividing stiffness and damping by 1000 is a trick I got from Matt 
        ...
        you never even see them happen in the preview. */ const shapeTransition =
      {
        pathLength: {
          ...pathAnimation,
          ...
          damping: isSpring
            ? pathAnimation.damping / 1e3
            : pathAnimation.damping,
        },
      };
    /* Add our own properties to the Path */ const pathLength =
      useMotionValue(0);
    const opacity = useTransform(pathLength, [0, 0.025], [0, 1]);
    const shapeProps = {
      variants: {
        start: { pathLength: from / 100 },
        end: { pathLength: to / 100 },
      },
      transition: shapeTransition,
    };
    /* Prevent animating or adjusting pathLength on the Canvas */ const isCanvas =
      RenderTarget.current() === RenderTarget.canvas;
    /* Just render the full connected Graphic on Canvas, when connected */ if (
      isCanvas
    ) {
...
/* Default Properties */ Animator.defaultProps = {
  animate: true,
  shouldLoop: false,
  loopOptions: "reverse",
  from: 0,
