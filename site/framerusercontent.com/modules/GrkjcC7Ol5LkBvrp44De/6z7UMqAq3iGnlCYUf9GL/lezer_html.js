...
import {
  ...
  ExternalTokenizer,
  LRParser,
} from "/framerusercontent.com/modules/MiFWyNBz6FaRwOOEYJ4H/nqI70oaHl4AhH5rIUMv4/lezer_lr.js";
import {
  styleTags,
  tags,
} from "/framerusercontent.com/modules/rOWwbZHN39cczduPnzmw/seDszB8oQDQQyH7PIqkK/lezer_highlight.js";
import { parseMixed } from "/framerusercontent.com/modules/CO9ruPlVDRTCLVInyri7/ye873rtFsAyZVFu9x2He/lezer_common.js";
var scriptText = 54;
...
var tagStart = new ExternalTokenizer(
  (input, stack) => {
...
var endTag = new ExternalTokenizer((input, stack) => {
  if (input.next == slash && input.peek(1) == greaterThan) {
    let selfClosing =
      stack.dialectEnabled(Dialect_selfClosing) ||
      inForeignElement(stack.context);
...
var textareaTokens = contentTokenizer(
  "textarea",
  textareaText,
...
function maybeNest(node, input, tags2) {
  ...
  for (let tag of tags2) {
    if (
      !tag.attrs ||
      tag.attrs(attrs || (attrs = getAttrs(node.node.parent.firstChild, input)))
    )
    ...
    if (id == Element && other.length) {
      ...
      if (tagName)
        for (let tag of other) {
          if (
            tag.tag == tagName &&
            (!tag.attrs || tag.attrs(attrs2 || (attrs2 = getAttrs(n, input))))
          ) {
...
export const __FramerMetadata__ = {
  exports: {
    ...
    parser: { type: "variable", annotations: { framerContractVersion: "1" } },
