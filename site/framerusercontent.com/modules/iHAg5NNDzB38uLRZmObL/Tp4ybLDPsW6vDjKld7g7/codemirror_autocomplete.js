...
var FuzzyMatcher = class {
  ...
  match(word) {
    ...
    if (direct == 0)
      return [
        word.length == this.pattern.length ? 0 : -100,
        0,
        this.pattern.length,
      ];
    ...
    let hasLower = /[a-z]/.test(word),
      wordAdjacent = true;
    for (
      let i = 0, e = Math.min(word.length, 200), prevType = 0;
      i < e && byWordTo < len;
    ...
    if (adjacentTo == len && adjacentStart == 0)
      return [
        -200 - word.length + (adjacentEnd == word.length ? 0 : -100),
        0,
        adjacentEnd,
      ];
    if (direct > -1)
...
var CompletionState = class {
  ...
  update(tr) {
    ...
    if (
      !open &&
      active.every((a) => a.state != 1) &&
      active.some((a) => a.hasResult())
    )
      active = active.map((a) =>
        a.hasResult() ? new ActiveSource(a.source, 0) : a
      );
...
var completionPlugin = /* @__PURE__ */ ViewPlugin.fromClass(
  class {
    update(update) {
      ...
      this.debounceUpdate = cState.active.some(
        (a) =>
          a.state == 1 && !this.running.some((q) => q.active.source == a.source)
      )
...
function wordCache(wordChars) {
  return (
    ...
    (wordCaches[wordChars] = /* @__PURE__ */ new WeakMap())
  );
}
...
var completionKeymapExt = /* @__PURE__ */ Prec.highest(
...
export const __FramerMetadata__ = {
  exports: {
    ...
    ifNotIn: { type: "function", annotations: { framerContractVersion: "1" } },
    completeAnyWord: {
      type: "variable",
      annotations: { framerContractVersion: "1" },
    },
    ...
    snippet: { type: "function", annotations: { framerContractVersion: "1" } },
    selectedCompletionIndex: {
      type: "function",
      annotations: { framerContractVersion: "1" },
    },
    ...
    ifIn: { type: "function", annotations: { framerContractVersion: "1" } },
    closeCompletion: {
      type: "variable",
      annotations: { framerContractVersion: "1" },
    },
