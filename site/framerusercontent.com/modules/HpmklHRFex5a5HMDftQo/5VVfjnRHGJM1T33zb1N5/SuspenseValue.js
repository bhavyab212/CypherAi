export class SuspenseValue {
  read() {
    const status = this.status; // The component did rerender and we already set the status below.
    if (status) {
      switch (
        status.type // If the promise is still pending we need to suspend again.
      ) {
        case "pending":
          throw status.promise; // If the promise is fulfilled we can return the value
        // synchronously.
        case "fulfilled":
          return status.value; // If the promise was rejected we can throw the error so we can
